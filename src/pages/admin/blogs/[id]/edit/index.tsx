...
import { requestUpdateBlog } from 'supabase/controller/Blogs.client';
import { UpdateBlog, getBlogById } from 'supabase/controller/Blogs.server';
...
import { DbBlogResponse, DbColor, DbMasterTagResponse } from 'types/Supabase';
...
interface AdminBlogsEditProps {
  data: DbBlogResponse;
  master_tags: DbMasterTagResponse[];
}
export default function AdminBlogsNew(props: AdminBlogsEditProps) {
  const { data: session } = useSession();
  const router = useRouter();
  const [tags, setTags] = useState<DbMasterTagResponse[]>(props.master_tags);
  const [filterTags, setFilterTags] = useState<string>('');

  const [blogTitle, setBlogTitle] = useState<string>(props.data.title);
  const [blogSummary, setBlogSummary] = useState<string>(props.data.summary);
  const [blogContent, setBlogContent] = useState<string>(props.data.content);
  ...
  const [blogTags, setBlogTags] = useState<DbMasterTagResponse[]>(props.data.tags ?? []);
  ...
  useEffect(() => {
    const isTitleFilled = blogTitle.length > 0 && blogTitle !== props.data.title;
    const isSummaryFilled = blogSummary.length > 0 && blogSummary !== props.data.summary;
    const isContentFilled = blogContent.length > 0 && blogContent !== props.data.content;
    const isThumbnailFilled = !!blogThumbnail;
    const isTagsFilled =
      (blogTags.length > 0 &&
        JSON.stringify(blogTags.sort((a, b) => a.id.localeCompare(b.id))).localeCompare(
          JSON.stringify(props.data.tags.sort((a, b) => a.id.localeCompare(b.id))),
        )) ||
      blogNewTags.length > 0;
  ...
  }, [blogTitle, blogSummary, blogContent, blogThumbnail, blogTags, blogNewTags, props.data]);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Upload thumbnail if there is any
      ...
      if (blogThumbnail) {
        ...
        payload.thumbnail_url = thumbnail_url.toString();
      }

      await requestUpdateBlog(props.data.id, payload, session);
  ...
  return (
    <AdminLayout
      seo={{
        title: 'Edit post',
        description: `Currently editing post "${props.data.title}"`,
      }}
      icon='Pencil'
      showTitle={false}
          ...
          <div className='flex h-fit w-full flex-col gap-4'>
            <div className='flex flex-col'>
              ...
              <div className='group relative aspect-[1200/630] h-fit w-full rounded-medium bg-background/25'>
                ...
                <Image
                  src={blogThumbnail ? URL.createObjectURL(blogThumbnail) : props.data.thumbnail_url ?? undefined}
                  alt='Thumbnail'
                  removeWrapper
...
): Promise<GetServerSidePropsResult<AdminBlogsEditProps>> {
  const { id } = context.params as { id: string };
  if (!id) return { notFound: true };

  const _data = getBlogById(id).catch(() => null);
  const _masterTags = await supabase.from('master_tag').select('*').order('name', { ascending: true });

  const [data, masterTags] = await Promise.all([_data, _masterTags]);
  if (!data) return { notFound: true };
  if (masterTags.error) throw masterTags.error;

  const rawContent = Buffer.from(data.content, 'base64').toString('utf-8');
  const dataWithContent: DbBlogResponse = {
    ...data,
    content: rawContent,
  };
